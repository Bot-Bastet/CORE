...
:   Node("ORB_SLAM3_ROS2")
{
    ...
    left_sub = std::make_shared<message_filters::Subscriber<ImageMsg> >(this, "camera/left/image_raw");
    right_sub = std::make_shared<message_filters::Subscriber<ImageMsg> >(this, "camera/right/image_raw");
    ...
    RCLCPP_INFO(this->get_logger(), "ORB_SLAM3 Stereo Node initialized.");
}
...
void StereoSlamNode::GrabStereo(const ImageMsg::SharedPtr msgLeft, const ImageMsg::SharedPtr msgRight)
{
    ...
    Sophus::SE3f Tcw = m_SLAM->TrackStereo(cv_ptrLeft->image, cv_ptrRight->image, msgLeft->header.stamp.sec + msgLeft->header.stamp.nanosec * 1e-9);
    ...
    {
        cv::circle(im, kp.pt, 2, cv::Scalar(0, 255, 0), -1);
    }
    
    std_msgs::msg::Header header = msgLeft->header;
    cv_bridge::CvImage cv_img(header, "bgr8", im);
    m_tracked_image_pub->publish(*cv_img.toImageMsg());
    ...
    if(!Tcw.translation().hasNaN())
    {
        ...
        t_msg.header.stamp = msgLeft->header.stamp;
        ...
        m_path.header.stamp = msgLeft->header.stamp;
    ...
    pc_msg.header.stamp = msgLeft->header.stamp;
